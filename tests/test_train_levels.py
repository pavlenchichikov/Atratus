...
def test_bars_with_no_atr_are_skipped_not_scored_as_zero():
    ...
    out = tl.eval_levels(s, dict(tl.DEFAULT_PARAMS),
                         sides=np.ones(WARMUP + 4, dtype=int))
...
def test_a_flat_side_issues_nothing():
    ...
    out = tl.eval_levels(s, dict(tl.DEFAULT_PARAMS),
                         sides=np.zeros(WARMUP + 4, dtype=int))
    assert out["n"] == 0
...
def test_the_gate_holds_when_the_effect_is_below_the_floor(monkeypatch):
    ...
    monkeypatch.setattr(tl, "eval_levels",
                        lambda s, p, sides=None: {"ret_per_bar": 0.00001 if p["k_stop"] != 2.0 else 0.0,
                                                  "n": 5, "wins": 3})
...
def test_the_gate_adopts_a_real_gain(monkeypatch):
    monkeypatch.setattr(tl, "sides_for", lambda s: np.ones(len(s["close"]), dtype=int))
    monkeypatch.setattr(tl, "eval_levels",
                        lambda s, p, sides=None: {"ret_per_bar": 0.004 if p["k_stop"] != 2.0 else 0.0,
                                                  "n": 5, "wins": 3})
    gate = tl.gate_policy(_by_asset(12, [100, 101]), {"k_entry": 0.5, "k_stop": 3.0})
...
def test_too_few_assets_is_never_an_adoption(monkeypatch):
    monkeypatch.setattr(tl, "sides_for", lambda s: np.ones(len(s["close"]), dtype=int))
    monkeypatch.setattr(tl, "eval_levels",
                        lambda s, p, sides=None: {"ret_per_bar": 0.05 if p["k_stop"] != 2.0 else 0.0,
                                                  "n": 5, "wins": 3})
...
def test_the_reward_no_longer_pays_for_scratching_every_trade():
    ...
    sides = _sides(closes)
    scratch = tl.eval_levels(s, {"k_entry": 0.5, "k_stop": 0.5}, sides=sides)
    sane = tl.eval_levels(s, {"k_entry": 0.5, "k_stop": 2.5}, sides=sides)
