...
- **Одна модель на актив, а не одна модель на рынок.** Для каждого актива обучается собственный ансамбль из четырёх членов (CatBoost, LSTM, Transformer, TCN), а чемпион выбирается по тому, насколько точно он предсказывает СЛЕДУЮЩИЙ БАР, walk-forward, с эмбарго против утечки. До 12.09.2026 его выбирал торговый Score; Score по-прежнему считается и показывается, но больше ничего не решает, потому что продукт это предсказание, а доходность это вопрос бэктеста. `GTRADE_CHAMPION_BASIS=score` возвращает прежнее правило в точности. Это центральное архитектурное решение, и числа ниже - его цена и его смысл.
  ...
  Из 838 активов **535 переобучены 2026-09-04 под принятым геномом `4_4_5` и продвинуты, а 303 остались на прежнем чемпионе**, потому что champion-challenger заменяет чемпиона только когда новая модель его обходит. С 12.09.2026 запас задаётся `GTRADE_CHAMPION_ACC_MARGIN` (0.005 точности), до этого было 0.2 по Score; первое переобучение на базисе точности продвинет все активы один раз, потому что ни у одного чемпиона, отобранного по старому правилу, точность не записана и сравнивать не с чем. Это работа механизма, а не сбой, и 211 из этих 303 - причина, по которой ниже появился раздел про сервинг поактивного генома.
...
- **Управление риском по замыслу.** Размер позиции по Келли, стопы по просадке, проверки секторной экспозиции и корреляций, а также индекс риска Талеба: размер уменьшается, когда волатильность актива поднимается в верхнюю часть его собственной истории, а на самом верху новые входы запрещены в обе стороны.
...
1. `data_engine.py` скачивает до 15 лет дневных и недельных котировок с Yahoo Finance и MOEX в `market.db` (SQLite). PEPE и SHIB берутся с Binance: Yahoo округляет их до 1e-6 и 1e-8, из-за чего 90% и 81% закрытий повторяли предыдущее, а один шаг округления выглядел как движение на 20%. В конце каждого прогона пересобирается `market_breadth`, признак ширины рынка, который больше ничто не обновляет.
2. `train_hybrid.py` строит признаки (см. выше), обучает ансамбль и сохраняет чемпиона вместе со скейлером и калибратором вероятностей, выбранного по walk-forward-точности на следующем баре. В отчёте о качестве рядом со старыми колонками появились `Ens_Acc` и `CB_Acc_Mean`: эти две усреднены по всем свёрткам, тогда как `CB_Acc` принадлежит только чемпионской.
...
- `/` - радар сигналов: BUY / SELL / WAIT по каждому активу с уверенностью, живой точностью, колонкой риска Талеба (0-100), панелью рыночной ширины в реальном времени и датчиками режима / fear-greed
- `/asset/BTC` - детализация по активу: графики цены и свечей, история сигналов, консенсус моделей, риск Талеба и value-вердикт Совета гуру (N/A для не-акций) с пересчётом по требованию
...
- `/risk` - интерактивный риск-менеджер: открыть / закрыть позиции, править и сохранять лимиты риска, останавливать / возобновлять торговлю, плюс watchlist риска Талеба
...
| `[4b]` Score basis | `1` сырой Score ансамбля, `2` вклад нейросетей, `3` собственный AUC сетей, `4` прирост сетей, `5` AUC ансамбля, `6` ТОЧНОСТЬ ансамбля, `7` объединённый t по сделкам. См. ниже |
...
### Насколько широко меряется гейт

Пункт меню `[4c]`. Финальный гейт месяцами шёл по зашитым четырнадцати активам,
а выбрать ширину можно было только из лаунчера кампании `[AL]`, но не из самого
`[RS]`.

Ширина это рычаг, потому что связывающее ограничение это гейт, а не поиск.
Разброс отложенных дельт Score около 3.74, поэтому наименьший различимый эффект
равен `2.8 * 3.74 / sqrt(n)` при пороге адопта 0.5:

```
n =  14   различимо 2.80      n =  80   различимо 1.17
n =  40   различимо 1.66      n = 440   различимо 0.50
```

Примерно до 440 активов гейт не видит эффекта размером с собственный порог
адопта, поэтому его отрицательный вердикт никогда не был отрицательным
результатом. Каждый вердикт печатает свою строку мощности с тем `n`, которое
было нужно; брать надо это число, а не выбирать на глаз.

Список строит `ab_build.py --search-gate N`, тот же вызов, что делает `[AL]`. Он
РАСШИРЯЕТ текущий гейт, а не заменяет его, поэтому прежние замеры остаются
сравнимыми, и выбрасывает то, что гейт не может честно оценить: активы со
слишком короткой историей и активы, чья цена котируется слишком грубо, чтобы
нести знак одного бара. Из 847 активов в `FULL_ASSET_MAP` пригодны 743.

Стоимость создают четыре члена ансамбля. На плечо: гейт из 14 активов это 33
минуты, 40 активов около 95, при этом более старая цифра в `PROGRESS_SEED` на
порядок больше, поэтому несколько сотен активов надо считать прогоном от ночи до
нескольких суток и смотреть реальное время в консоли прогресса. Скрин только на
CatBoost стоит около 12с на актив и предлагается в `[4c]` вторым вопросом, но он
ставит вероятности всех сетей в константу 0.5: кандидат, который помогает только
сетям, читается на нём ровно нулём.
...
В основе сырые данные - 79 полей в двенадцати блоках: цена и движение, где
цена стоит в собственных единицах волатильности, прошедший год против индекса,
оборот, рынок вокруг, режим и сектор, отчётные фундаментальные показатели,
заголовки из многих изданий, календарь и ключевая ставка, а также его
собственные прошлые суждения по этому активу. Чужих вердиктов в досье нет:
вердикт совета Guru убран 2026-09-24, потому что вердикт это мнение, как бы
хорошо его ни оценивали. Сверх досье аналитик сам запрашивает данные, см.
[Какие источники он запрашивает](#какие-источники-он-запрашивает).
...
### Какие источники он запрашивает

Досье это нижняя планка, а не потолок. Прежде чем вынести суждение, аналитик
**запрашивает дополнительные сырые данные**, и по умолчанию обязан это сделать:
первый ответ должен быть запросом, а суждение, пришедшее до любого запроса,
один раз возвращается обратно (`GTRADE_ANALYST_REQUIRE_TOOL=0` делает запросы
необязательными). Предоставленная сама себе, gemma4:26b судила SBER только по
досье.

| Инструмент | Что возвращает | Для каких активов |
|---|---|---|
| `news_search` | заголовки по запросу модели, из многих изданий | для всех |
| `macro_series` | статистика FRED: доходности США 2y/10y и кривая, ставка ФРС, CPI, безработица, инфляционные ожидания, спред high yield, Brent, индекс доллара, с изменениями за 1м/3м/1г | для всех |
| `attention` | дневные просмотры статьи в английской Википедии, последняя неделя против предыдущего квартала | для всех |
| `company_financials` | выручка, чистая прибыль, операционный денежный поток, EPS и число акций из отчётности в SEC (XBRL) | бумаги США |
| `insider_filings` | сделки инсайдеров, РАСКРЫТЫЕ в SEC по форме 4 | бумаги США |
| `options_positioning` | открытый интерес и объём пут/колл на две ближайшие экспирации | бумаги США |
| `crypto_derivatives` | бессрочные фьючерсы Binance: funding, открытый интерес и его изменение за 7 и 30 дней, соотношение лонг/шорт | крипта |

В промпте показываются только те инструменты, которые могут ответить по
данному активу. Когда SBER предлагались отчётность SEC и опционы США, модель
потратила два запроса из шести на пустые ответы. Подразумеваемой волатильности
в опционах нет намеренно: вне сессии Yahoo считает её по пустому стакану, и у
NVDA она показывала 2% при реальных 40.

**Новости это материал, но не фундамент.** Любой блок заголовков, и в досье, и
из `news_search`, собирается одинаково: не больше двух заголовков от одного
издания, ничего старше 90 дней, заголовок обязан называть компанию (поиск, где
компания не названа, например ставка ЦБ, должен совпадать хотя бы с половиной
своих слов), собственный канал компании помечается `company_own`, а
`news_publishers` показывает, сколько независимых изданий на самом деле стоит
за блоком. Промпт велит считать одно издание заявлением, а два и больше ближе
к факту, и сверять каждую новость с цифрами. Однажды SBER досталось шесть
статей, четыре из них про его же экспансию в Индию, а поиск Google News по
"Sberbank crypto" вернул Kalshi и статью про устройство Ethereum. Именно это
эти правила и убирают.

Инструмент может вернуть материал, но не чужой вывод: консенсус аналитиков,
целевые цены, рейтинги брокеров и собственный совет Guru исключены решением
владельца, и `tools.register()` падает, а не принимает такой источник. Каждый
вызов и его результат записываются в строку суждения
(`analyst_log.tool_calls_json`), а перемотанный прогон (`--as-of`, `--back`)
получает только инструменты, которые умеют отвечать на прошлую дату.

Один ответ может запросить несколько инструментов сразу,
`{"tools": [{...}, {...}]}`, потому что каждый раунд это полный вызов модели.
`GTRADE_ANALYST_TOOL_CALLS` ограничивает число источников на суждение (по
умолчанию 6), `GTRADE_ANALYST_TOOL_ROUNDS` число раундов (по умолчанию 2).
Для `company_financials` и `insider_filings` нужна `GTRADE_SEC_CONTACT` с
адресом почты: без контакта в User-Agent SEC отвечает 403.
...
Горизонт задаётся в торговых днях. По умолчанию прогон судит **1 и 20** дней
(`GTRADE_ANALYST_HORIZONS`, `--horizons`). Каждый горизонт это отдельный вопрос
и отдельный вызов модели, а карточка актива показывает последнее суждение по
каждому горизонту с подписью и датой, когда оно проверится.
`GTRADE_ANALYST_MODEL` задаёт собственную модель аналитика отдельно от модели
директора исследований, так что большая и медленная модель не тормозит поиск.

### Слои на видеокарте, под свою карту

Сколько слоёв модели класть на видеокарту, Ollama решает сама, и на маленькой
карте может решить "ни одного". Она держит свободными около 2.2 ГБ и
резервирует ещё 1.3 ГБ под визуальный энкодер gemma4, поэтому на RTX 2050 с
4 ГБ она положила на GPU **0 из 31** текстовых слоёв, и аналитик считал на
процессоре.

`GTRADE_OLLAMA_NUM_GPU` задаёт это число вручную. Значение своё для каждой
машины: пишите его в свой `.env`, а не в репозиторий. Замер на gemma4:26b
(Q4_K_M) при контексте 32k:

| RTX 2050, 4 ГБ | результат |
|---|---|
| не задано (Ollama решает: 0) | 3.3 токена/с |
| `4` | **5.1 токена/с**, занято 3.6 ГБ видеопамяти |
| `5` и больше | не загружается: не помещается контекст |

Как подобрать число для своей карты:

1. Начните примерно с `(видеопамять в ГБ - 2) / 0.5` для gemma4:26b: 4 на
   4 ГБ, 12 на 8 ГБ, 20 на 12 ГБ. На 24 ГБ ставьте 99, то есть все слои.
2. Запустите одно суждение и найдите в `%LOCALAPPDATA%\Ollama\server.log`
   строку `offloaded N/31 layers to GPU`.
3. Увеличивайте, пока загрузка не упадёт, и оставьте последнее рабочее
   значение. Меньший контекст (`OLLAMA_CONTEXT_LENGTH` на стороне Ollama)
   освобождает место под дополнительные слои.

Значение, которое помещается только на свободной карте, оставлять безопасно:
если карту держит обучение и загрузка падает, тот же запрос уходит повторно
без `num_gpu` и отвечает на процессоре, медленнее, но отвечает.
...
| Клавиша | Запускает | Примечание |
| --- | --- | --- |
| `TP` | `train_timing.py` | Фитит правила тайминга (Stage A). Спрашивает бюджет поиска, терпение до рестарта и метрику. |
| `TB` | `train_timing.py --stage b` | Fitted-Q претендент. Спрашивает итерации Q, раунды сбора данных и одиночный или двойной Q. |
...
Самый важный ответ это базис счёта. На net-базисе (`net_auc`, `net_gain`, `ens_auc`, `ens_acc`) скрин выключается, а иллюминация тренирует настоящие сети, поэтому базис решает, какие геномы становятся элитами. На `raw` и `neural` поиск иллюминируется CatBoost-скрином, и базис пересчитывает только финальный гейт. `ens_acc` это базис ТОЧНОСТИ, добавлен 2026-09-12 и стоит умолчанием кампании: он считает усреднённую по фолдам точность ансамбля, то есть ту же величину, по которой теперь отбирается сам чемпион (`GTRADE_CHAMPION_BASIS=acc`), так что поиск, принятие и чемпион отвечают на один вопрос. Колонка `CB_Acc` это НЕ она: она принадлежит чемпионскому фолду, который под базисом `score` выбран argmax-ом, и её верхний квартиль дал 0.6356 офлайн против 0.4820 живьём. Про мощность вопроса нет: она выводится из кампании и не входит в ключ кеша оценок, так что её смена между прогонами сравнивала бы закешированную базу с иначе обученными кандидатами.
...
- **`entry_source`** - от какого входа считался стоп, `fill` или `signal`. Это
  два разных ответа, и по цене в строке их не различить, поэтому лист говорит
  прямо.

**Скажи системе, где ты на самом деле вошёл.** Ордера ставятся руками, и пока
исполнение не записано, подтянутый стоп считается от бара, на котором
повернулся СИГНАЛ, по закрытию того бара. Ордер, поставленный рукой,
исполняется в другой день и по другой цене, а стоп это лучший
`close - k * ATR`, увиденный ПОСЛЕ входа, поэтому вход тремя днями позже не
может претендовать на тот максимум, на который претендует реконструкция.
...
Одна открытая позиция на актив, и это частичный уникальный индекс в базе, а не
договорённость, которую каждый вызывающий обязан помнить. Открытое исполнение
заменяет реконструированный сегмент в листе; если ничего не записано, ничего и
не меняется, то есть журнал добавочный, и установка, которая им не пользуется,
ведёт себя как раньше. Это намеренно не менеджер позиций: ни P&L, ни размера,
ни сверки с риск-книгой. Он отвечает на один вопрос, что открыто и с какого
числа, потому что именно он нужен уровням.
...
### Одно число для фита и для гейта

`GTRADE_TIMING_OBJECTIVE` задаёт, что фит тайминга максимизирует И что судит
его гейт, и смысл в том, что это одна настройка, а не две.

| значение | что меряет | порог адопции |
| --- | --- | --- |
| `net` (по умолчанию) | чистая доходность на БАР среза, то, что накапливает счёт | 0.0002 |
| `score` | `profit - 0.5 maxDD + 0.1 winrate + 2.0 Sharpe` | 0.5 |

До 5 сентября 2026 поиск максимизировал чистую доходность, а гейт ранжировал
композит, то есть фиттер лез на один холм, а отбор оставлял себе другой. Это та
же ошибка, что была найдена с базисом поиска в августе, только этажом выше, и
именно поэтому подобранная политика тайминга могла пройти собственный фит и
ничего не говорить о том, сколько она зарабатывает. `score` остаётся
достижимым, потому что ставка не видит просадку вообще и потому что все
измерения до той даты сделаны на нём.

Метрику-ставку можно выиграть, ПРОСТО НЕ ТОРГУЯ, поэтому вердикт теперь
печатает рядом число сделок обеих рук. Политика, выигравшая тем, что стоит в
стороне, это честный ответ, но он должен быть виден в отчёте, а не обнаружен в
проде.

### Почему поиск перезапускается

`[TP]` спрашивает терпение до рестарта, по умолчанию 40, а 0 воспроизводит
односкоростной поиск, каким он был до 6 сентября 2026. Пункт существует из-за
того, что этот поиск, как выяснилось, делал.

Сепарабельная ES ранжирует кандидатов и стягивает sigma к победителям. На
плоском шумном ландшафте она поэтому ранжирует ШУМ, и sigma падает с 0.25
диапазона каждого параметра до своего пола 0.01 **к 60-й оценке из 400**, после
чего каждый оставшийся кандидат пересэмплирует одну точку. Фит на 400 оценках
по 418 активам в итоге вернул ШЕСТОГО кандидата, розыгрыш, сделанный до первой
адаптации и байт-в-байт совпавший с тем, что дал прогон на 12 активах с тем же
сидом, потому что эмиттер копит первые шесть оценок без адаптации. Остальные
394 не купили ничего.

С терпением поиск переоткрывается вокруг лучшего известного вектора всякий раз,
когда валидационный срез не улучшался столько оценок подряд, и это
переиспользует бюджет вместо того, чтобы его жечь. Рестарт живёт в
`train_timing.py`, а не в `core/ar_rl.py`, и намеренно: тот эмиттер общий с
исследовательским агентом, и его поведение не должно меняться под кампанией,
которая уже идёт.
...
Про то, КАК он подбирается, стоит знать две вещи: обе спрашиваются пунктом
`[TB]` и обе выключаются, если нужно воспроизвести любой прежний фит.

**Раунды сбора данных** (`--rounds`, по умолчанию 3). Первый раунд пишет
переходы под принятыми правилами, каждый следующий под тем Q, который выдал
предыдущий, и данные накапливаются, а не заменяются. Здесь это дёшево по
причине, которой в большинстве задач обучения с подкреплением нет: среда это
детерминированный реплей по зафиксированным барам без рыночного влияния,
поэтому политику, которая никогда не работала, можно прокатить ТОЧНО, а не
оценить. Значит состояния, в которые обученный Q реально попадает, лежат в его
собственных обучающих данных уже со второго раунда, вместо того чтобы
экстраполироваться из состояний, куда заходили правила. `--rounds 1` это
односборный фит, каким он был до 5 сентября 2026.

**Двойная оценка** (`--single-q` её выключает). Две регрессии обучаются на
непересекающихся половинах АКТИВОВ, и в целевом значении Беллмана одна из них
выбирает следующее действие, а вторая его оценивает, симметрично в обе стороны.
Делить по активу, а не по строке, здесь и есть суть: две строки одной
траектории соседние и по состоянию, и по бару, поэтому построчное деление
протекает ровно тем, от чего вторая модель должна быть независима. Здесь это
важнее, чем в большинстве применений, из-за размера награды: около одного
базисного пункта края на бар при дневной доходности, разброс которой на два
порядка больше, так что почти любая разница, которую обычный максимум находит
между двумя действиями, это шум выборки, а одиночный Q этот шум и выбирает, и
принимает на веру. Двойной фит пишет два файла, `timing_fqi.cbm` и
`timing_fqi_b.cbm`; если второй отсутствует или не читается, обслуживание
откатывается к правилам, а не к половине модели.
...
| `GTRADE_TIMING_OBJECTIVE` | что фит тайминга максимизирует и что судит его гейт: `net` (по умолчанию) или `score` |
...
| `GTRADE_ANALYST_TOOL_CALLS` | сколько дополнительных источников одно суждение вправе запросить (по умолчанию 6, `0` запрещает запросы) |
| `GTRADE_ANALYST_TOOL_ROUNDS` | за сколько раундов (по умолчанию 2). Каждый раунд это ещё один полный вызов модели, на локальной 26b ещё 10-25 минут |
| `GTRADE_ANALYST_REQUIRE_TOOL` | `1` (по умолчанию): первый ответ обязан запросить данные до суждения; `0` делает запрос необязательным |
| `GTRADE_ANALYST_MODEL` | собственная модель аналитика, отдельно от директора исследований (например `gemma4:26b`) |
| `GTRADE_ANALYST_HORIZONS` | горизонты в торговых днях, если `--horizons` не задан (по умолчанию `1,20`) |
| `GTRADE_OLLAMA_NUM_GPU` | сколько слоёв модели Ollama кладёт на видеокарту; не задано - решает сама Ollama. Своё для каждой машины, см. [Слои на видеокарте](#слои-на-видеокарте-под-свою-карту) |
| `GTRADE_SEC_CONTACT` | почта для User-Agent, которую требует SEC; без неё `company_financials` и `insider_filings` возвращают инструкцию, а не 403. В репозиторий не попадает: адрес ваш, а не проекта |
...
fills.py              запись того, что реально исполнено у брокера
...
tests/                pytest-набор (1994 теста, ~2 мин)
