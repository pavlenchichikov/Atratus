...
- **Одна модель на актив, а не одна модель на рынок.** Для каждого актива обучается собственный ансамбль из четырёх членов (CatBoost, LSTM, Transformer, TCN), а чемпион выбирается walk-forward-бэктестом с комиссиями, проскальзыванием и эмбарго против утечки. Это центральное архитектурное решение, и числа ниже - его цена и его смысл.
  ...
  На 2026-09-03: **847 тикеров в `FULL_ASSET_MAP`, 827 из них обучены и актуальны, 20 ждут первого обучения.** Разрыв - не деградация. Тикер попадает в карту в тот момент, когда за ним стоит следить, а обучить его - отдельное дорогое действие, поэтому карта всегда идёт впереди `models/`. Эти двадцать - недавние листинги и добавления в индексы (`ARB`, `CRWV`, `NBIS`, `SNOW`, `GEV`, `TAO`, `MBNK`, `RENI`, `KLVZ`, `DIAS`, `PRMD`, `EGX30`, `WIG20` и ещё семь); закрывает разрыв пункт `[5F]`.
