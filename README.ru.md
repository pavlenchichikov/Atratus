...
Пункт меню `[4c]`, `GTRADE_AR_HELDOUT`. Три значения: `prod` (по умолчанию,
продовый холдаут из 14 активов), `neural` (14 активов, чей стакер опирается на
сети, диагностика и смещён по построению) и `all`.

`all` это каждый актив, у которого уже есть полный чемпион, минус все активы,
которые видел сам поиск. Замер 2026-09-08: 823 из 847 активов в
`FULL_ASSET_MAP`; `selection_assets()` и `tier_assets()` вычитаются внутри
`all_trained_assets`, поэтому непересечение, нужное лестнице тиров,
обеспечивается кодом, а не памятью.

Смысл в том, что связывающее ограничение это гейт, а не поиск. Разброс dScore
по холдауту из 14 активов равен 3.74, поэтому эффект +0.5 становится виден
только на 439 активах, и адоптов нет уже месяцами. 823 актива сжимают шум в
`sqrt(823/14)`, то есть в 7.7 раза, и обучать ничего не надо: чемпионы уже
лежат на диске.

Загвоздка в стоимости замера, и времена прогонов записаны в `PROGRESS_SEED`.
Один холдаут на 14 активов занимает около 35000с, то есть 2555с на актив, так
что 823 актива на полной конфигурации из четырёх членов это примерно 24 дня на
плечо. Скрин только на CatBoost стоит около 12с на актив, поэтому те же 823
актива обходятся примерно в 2.7 часа на плечо. Поэтому при выборе `all` меню
задаёт второй вопрос и по умолчанию ставит `GTRADE_SCREEN_ONLY=1`.

Значит `all` это ночной прогон одного финалиста в конце кампании, на кандидате
со стороны CatBoost. На нейросетевом кандидате плечо только с CatBoost не меряет
ничего, а полная конфигурация на такой ширине неподъёмна.
