...
- **Un modèle par actif, et non un modèle pour le marché.** Chaque actif entraîne son propre ensemble de quatre membres (CatBoost, LSTM, Transformer, TCN), et le champion est choisi par un backtest walk-forward tenant compte des commissions, du slippage et d'un embargo contre les fuites de données. C'est la décision d'architecture centrale, et les chiffres ci-dessous en sont le coût et la contrepartie.
  ...
  Au 2026-09-05 : **847 tickers dans `FULL_ASSET_MAP`, dont 838 entraînés, et 9 en attente d'un premier entraînement.** Cet écart n'est pas une dégradation. Un ticker entre dans la carte dès qu'il mérite d'être suivi, alors que l'entraîner est un acte distinct et coûteux ; la carte devance donc toujours `models/`. Ces neuf-là sont des cotations récentes et des ajouts d'indices ; l'entrée `[5F]` du menu comble l'écart.

  Sur ces 838, **535 ont été réentraînés le 2026-09-04 sous le génome adopté `4_4_5` puis promus, et 303 ont gardé le champion qu'ils avaient**, car champion-challenger ne remplace un champion que si le nouveau modèle le dépasse de 0,2. C'est le mécanisme qui fonctionne, non une panne ; 211 de ces 303 expliquent le paragraphe sur le service du génome par actif.
...
**Un génome vit dans un processus, et le service n'en est qu'un.** Jusqu'au 2026-09-05, l'adoption par actif ne valait qu'à l'entraînement : `core/adopted.genome_for_assets` résout un génome par processus à partir de `GTRADE_ASSETS`, ce qui convient à un entraîneur, puisque `train_chunked.py` groupe les actifs par génome et lance un processus par groupe. Le service, lui, ne nomme aucun actif - le radar balaie toute la carte en un seul processus - et recevait donc toujours le génome global. Cette couture est restée sans conséquence jusqu'à ce qu'une adoption change le **jeu de variables** : le 2026-09-04 le génome global est passé de `A` (sept variables construites) à `4_4_5` (une seule), champion-challenger a conservé à juste titre le champion en place sur 303 actifs, et 211 de ces champions sont ajustés sur les variables de `A`. Le passage suivant du radar les a tous ignorés :
...
La décision était à la fois juste et inexécutable : au Score l'ancien champion l'emporte, mais il ne peut produire aucune prédiction, ce qu'aucune comparaison de scores ne voit. Le service repointe désormais l'environnement actif par actif. `serving_keys()` dit quelles clés il a le droit de toucher - tout ce que fixe un génome adopté, moins ce qui se trouvait déjà dans l'environnement au moment de l'adoption, pour qu'une valeur exportée dans le shell l'emporte toujours - et `apply_for()` met en place le génome d'un actif en **supprimant** toute clé que ce génome ne fixe pas : sans cette suppression, le premier actif traité figerait le génome pour tous les suivants, puisque `apply()` n'écrase jamais une clé déjà posée. Les 211 sont épinglés sur `A` avec la raison écrite telle quelle, à savoir une contrainte d'exécutabilité et non une mesure indiquant que `A` y serait meilleur.

**L'ordre compte autour d'une adoption**, car `[AG]` change le jeu de variables dès son exécution : `[4]` données, puis `[LC]` rapprochement, puis `[AG]` adopter, puis `[5C]` réentraîner. Le rapprochement reste sûr des deux côtés de cette ligne, `performance_tracker.py` n'important aucun modèle ; la prédiction, elle, ne l'est pas.
