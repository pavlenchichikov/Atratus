...
- **Ein Modell je Wert, nicht ein Modell für den Markt.** Jeder Wert trainiert sein eigenes Ensemble aus vier Mitgliedern (CatBoost, LSTM, Transformer, TCN), und der Champion wird durch einen Walk-Forward-Backtest mit Gebühren, Slippage und einem Embargo gegen Datenlecks bestimmt. Das ist die zentrale Entwurfsentscheidung, und die folgenden Zahlen sind ihr Preis und ihr Ertrag.
  ...
  Stand 2026-09-05: **847 Ticker in `FULL_ASSET_MAP`, davon 838 trainiert, 9 warten auf ein erstes Training.** Diese Lücke ist kein Verfall. Ein Ticker kommt in die Karte, sobald er beobachtenswert ist, während ihn zu trainieren ein eigener, teurer Vorgang bleibt; die Karte läuft `models/` deshalb immer voraus. Diese neun sind junge Notierungen und Indexaufnahmen; geschlossen wird die Lücke über den Menüpunkt `[5F]`.

  Von diesen 838 wurden **535 am 2026-09-04 unter dem adoptierten Genom `4_4_5` nachtrainiert und befördert, 303 behielten den Champion, den sie hatten**, denn champion-challenger ersetzt einen Champion nur, wenn ihn das neue Modell um 0,2 schlägt. Das ist der Mechanismus bei der Arbeit, kein Fehler; 211 dieser 303 sind der Grund für den Absatz über das Ausliefern eines Genoms je Wert.
...
**Ein Genom lebt in einem Prozess, und das Ausliefern ist genau einer.** Bis zum 2026-09-05 war die Adoption je Wert ein reiner Trainingsmechanismus: `core/adopted.genome_for_assets` löst je Prozess ein Genom aus `GTRADE_ASSETS` auf, was für einen Trainer stimmt, da `train_chunked.py` die Werte nach Genom gruppiert und je Gruppe einen Prozess startet. Das Ausliefern nennt keine Werte - der Radar durchläuft die ganze Karte in einem Prozess - und bekam deshalb immer das globale Genom. Diese Naht blieb folgenlos, bis eine Adoption den **Merkmalssatz** änderte: am 2026-09-04 wechselte das globale Genom von `A` (sieben konstruierte Merkmale) auf `4_4_5` (eines), champion-challenger behielt auf 303 Werten zu Recht den amtierenden Champion, und 211 dieser Champions sind auf die Merkmale von `A` angepasst. Der nächste Radarlauf übersprang jeden einzelnen:
...
Die Entscheidung war zugleich richtig und nicht ausführbar: nach Score gewinnt der alte Champion, doch er kann überhaupt keine Vorhersage liefern, und das sieht kein Vergleich von Scores. Das Ausliefern richtet die Umgebung nun je Wert neu aus. `serving_keys()` sagt, welche Schlüssel es anfassen darf - alles, was ein adoptiertes Genom setzt, abzüglich dessen, was zum Zeitpunkt der Adoption bereits in der Umgebung stand, damit ein in der Shell exportierter Wert weiterhin gewinnt - und `apply_for()` setzt das Genom eines Wertes und **entfernt** jeden Schlüssel, den dieses Genom nicht setzt: ohne das Entfernen würde der erste bearbeitete Wert das Genom für alle folgenden festschreiben, da `apply()` einen bereits gesetzten Schlüssel nie überschreibt. Die 211 sind auf `A` festgelegt, und der Grund steht so da, wie er ist: eine Ausführbarkeitsbedingung, keine Messung, dass `A` dort besser wäre.

**Um eine Adoption herum zählt die Reihenfolge**, denn `[AG]` ändert den Merkmalssatz im Moment des Aufrufs: `[4]` Daten, dann `[LC]` Abgleich, dann `[AG]` adoptieren, dann `[5C]` nachtrainieren. Der Abgleich ist auf beiden Seiten dieser Linie sicher, da `performance_tracker.py` kein Modell importiert; die Vorhersage ist es nicht.
